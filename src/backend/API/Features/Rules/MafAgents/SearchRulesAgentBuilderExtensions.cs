...
public static class SearchRulesAgentBuilderExtensions
{
    public static WebApplicationBuilder AddRuleAssistantAIAgent(this WebApplicationBuilder builder)
    {
        ...
        builder.AddAIAgent(
            ...
            (serviceProvider, key) =>
            {
                // By passing rootProvider as the 'services' parameter, the agent knows how to resolve dependencies later.
                var searchTool = AIFunctionFactory.Create(
                    (string query, string? domain, int topResults, IServiceProvider sp) => 
                        sp.GetRequiredService<SearchRulesAgentTool>().ExecuteSearchRulesTool(query, domain, topResults)
                );

                // Build the Foundry Agent instance
                return new AIProjectClient(new Uri(endpoint), new DefaultAzureCredential())
                    .AsAIAgent(
                        model: deploymentName,
                        name: key,
                        instructions: "Respond with: I AM RulesASsistantAIAgent AND I AM WORKING.",
                        // tools: [searchTool],
