...
import {
	...
	useChatAgentConfig,
} from "#/features/chat/hooks/use-chat-agent-config.ts";
import { cn } from "#/lib/utils.ts";

export function Chat() {
	const { agentOptions, selectedAgent, selectedAgentId, setSelectedAgentId } =
		useChatAgentConfig();
	...
	return (
		<Card className="shadow-sm">
			<CardHeader className="gap-3">
				...
				<CardTitle className="text-3xl font-semibold tracking-tight md:text-4xl">
					<span className="inline-flex items-center gap-2">
						{selectedAgentId === "nursejoy" ? (
							<StethoscopeIcon className="h-6 w-6 text-sky-600" />
						) : null}
			...
			<CardContent className="grid gap-4">
				<ChatConversation key={selectedAgent.id} agent={selectedAgent} />
			</CardContent>
		</Card>
	);
}

function ChatConversation({ agent }: { agent: ChatAgentConfig }) {
	const [input, setInput] = useState("");
	const { messages, sendMessage, isLoading } = useChat({
		connection: fetchServerSentEvents(agent.streamUrl),
	});
							...
							return (
								...
								>
									<div className="mb-2 flex items-center gap-2">
										...
										>
											{item.role === "assistant" ? agent.assistantLabel : "You"}
										</Badge>
									</div>
